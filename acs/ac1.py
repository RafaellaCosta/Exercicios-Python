...
def calcule_armstrong(n):
    palavra = str(n)
    qtd_digitos = len(palavra)
    soma = 0

    for x in range(0, qtd_digitos):
        soma += int(palavra[x]) ** qtd_digitos
...
def eh_quase_armstrong(n):
    ...
    if n > 0 and n != soma and (n == (soma - 1) or (soma + 1) == n):
        return True
    else:
        return False
